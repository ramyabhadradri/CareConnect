...
function PredictDisease() {
  ...
  const [disease, setDisease] = useState(null); // State to store the predicted disease
  ...
  const handleSubmit = async (event) => {
    ...
    setDisease(null); // Clear previous predictions
    ...
    try {
      ...
      setDisease(response.data.disease); // Update state with the predicted disease
      setShowModal(true); // Show the modal with the prediction
      setSelectedSymptoms([]);
    } catch (error) {
      setError('Error making prediction. Please try again.'); // Set error message
    }
  };
